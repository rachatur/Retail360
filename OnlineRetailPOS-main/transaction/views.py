...
def endTransaction(request,type,value):
    try:
        customer_number = request.session.get('customer_number', None)
        customer_id = request.session.get('customer_id', None)
        customer_name = request.session.get('customer_name', None)

        customer = None

        # Ensure the customer number exists before processing further
        if customer_number:
            # If customer_id is provided, try to update the existing customer record
            if customer_id and customer_id != 'not_provided':
                # Check if the customer exists and update details if so
                if Customer.objects.filter(customer_id=customer_id).exists():
                    customer = Customer.objects.get(customer_id=customer_id)
                    customer.customer_contact = customer_number
                    customer.customer_name = customer_name if customer_name else customer.customer_name  # Keep existing name if not provided
                    customer.save()
            else:
                # If no customer_id, create a new customer (handle customer_name as well)
                customer = Customer.objects.create(
                    customer_contact=customer_number,
                    customer_name=customer_name if customer_name else 'No Name'  # Use default name if not provided
                )
            request.session.pop('customer_number', None)
...
def addTransaction(user,payment_type,total,cart,value,customer):
    ...
    return transaction.objects.create( transaction_id = transaction_id , transaction_dt = datetime.strptime(transaction_id[:-6],'%Y%m%d%H%M%S'),
            user = user, customer=customer, total_sale= total, sub_total = round(total-tax_total,2),tax_total=tax_total, deposit_total = deposit_total,
